  ...
  if (obj_arr.length === 0) {
    ...
    MongoClient.connect(
      ...
      function (err, db) {
        ...
        dbo
          .collection('matches')
          .insertMany(obj_arr, { ordered: false }, function (err, res) {
            if (err) {
...
function displayTime(currentFrame) {
  ...
  return (
    h + ':' + showTwoDigits(m) + ':' + showTwoDigits(s) + ':' + showTwoDigits(f)
  );
}
...
exports.findAll = (req, res) => {
  ...
  let myConversionRate;
  myOppConversionRate = 0;
  let myDamage = 0;
  let myOppDamage = 0;
  ...
  // death direction
  let deathDirectionCharUsage = Array.from({ length: 4 }, (e) =>
    Array(26).fill(0)
  );

  let deathDirectionOppCharUsage = Array.from({ length: 4 }, (e) =>
    Array(26).fill(0)
  );
  ...
  let throwCountOppCharUsage = Array.from({ length: 4 }, (e) =>
    Array(26).fill(0)
  );

  // backward forward neutral fail
  let groundTechCountCharUsage = Array.from({ length: 4 }, (e) =>
    Array(26).fill(0)
  );
  let groundTechCountOppCharUsage = Array.from({ length: 4 }, (e) =>
    Array(26).fill(0)
  );
  ...
  function accumulateStats(connect_code, res) {
    ...
    if (res.metadata.winner === connect_code) {
      ...
      for (let j = 0; j < res.players.length; j++) {
        if (res.players[j].code === connect_code) {
          ...
          myTotalStockDifferential +=
            res.players[j].killCount - res.players[j].deathCount;
          // 4 Stocks
          if (
            res.players[j].killCount === 4 &&
            res.players[j].deathCount === 0
          ) {
            myFourStocks++;
          }
        } else {
      ...
      res.metadata.winner !== connect_code &&
      res.metadata.winner !== 'INCOMPLETE' &&
      res.metadata.winner !== 'DRAW'
    ) {
      myTotalLosses++;
    ...
    for (let j = 0; j < res.players.length; j++) {
      if (res.players[j].code === connect_code) {
        ...
        myCreditedKills += Number.isInteger(res.players[j].creditedKillCount)
          ? res.players[j].creditedKillCount
          : 0;
        ...
        grabCountSuccessCharUsage[res.players[j].characterId] +=
          res.players[j].throwCount.up +
          res.players[j].throwCount.forward +
          res.players[j].throwCount.back +
          res.players[j].throwCount.down;
        grabCountWhiffCharUsage[res.players[j].characterId] +=
          res.players[j].grabCount.fail;

        throwCountCharUsage[0][res.players[j].characterId] +=
          res.players[j].throwCount.up;
        throwCountCharUsage[1][res.players[j].characterId] +=
          res.players[j].throwCount.forward;
        throwCountCharUsage[2][res.players[j].characterId] +=
          res.players[j].throwCount.back;
        throwCountCharUsage[3][res.players[j].characterId] +=
          res.players[j].throwCount.down;

        groundTechCountCharUsage[0][res.players[j].characterId] +=
          res.players[j].groundTechCount.backward;
        groundTechCountCharUsage[1][res.players[j].characterId] +=
          res.players[j].groundTechCount.forward;
        groundTechCountCharUsage[2][res.players[j].characterId] +=
          res.players[j].groundTechCount.neutral;
        groundTechCountCharUsage[3][res.players[j].characterId] +=
          res.players[j].groundTechCount.fail;

        wallTechCountSuccessCharUsage[res.players[j].characterId] +=
          res.players[j].wallTechCount.success;
        wallTechCountFailCharUsage[res.players[j].characterId] +=
          res.players[j].wallTechCount.fail;

        if (j === 0) {
          sdCharUsage[res.players[j].characterId] +=
            res.players[j].deathCount - res.players[1].creditedKillCount
              ? res.players[j].deathCount - res.players[1].creditedKillCount
              : 0;
          deathUsage[res.players[j].characterId] += Number.isInteger(
            res.players[1].creditedKillCount
          )
            ? res.players[1].creditedKillCount
            : 0;
        } else if (j === 1) {
          sdCharUsage[res.players[j].characterId] +=
            res.players[j].deathCount - res.players[0].creditedKillCount
              ? res.players[j].deathCount - res.players[0].creditedKillCount
              : 0;
          deathUsage[res.players[j].characterId] += Number.isInteger(
            res.players[0].creditedKillCount
          )
            ? res.players[0].creditedKillCount
            : 0;
        }
        ...
        for (let k = 0; k < res.players[j].conversions.length; k++) {
          var currentConversion =
            res.players[j].conversions[k].endPercent -
            res.players[j].conversions[k].startPercent;
          ...
          if (res.players[j].conversions[k].didKill) {
            if (res.players[j].conversions[k].moves[0]) {
              myMoveUsageArr.killMoves[res.players[j].characterId][
                res.players[j].conversions[k].moves[
                  res.players[j].conversions[k].moves.length - 1
                ].moveId
              ]++;
            }
          ...
          switch (res.players[j].conversions[k].openingType) {
            case 'neutral-win':
              if (res.players[j].conversions[k].moves[0]) {
                myMoveUsageArr.neutralWinMoves[res.players[j].characterId][
                  res.players[j].conversions[k].moves[0].moveId
                ]++;
              }
              break;
            case 'counter-attack':
              if (res.players[j].conversions[k].moves[0]) {
                myMoveUsageArr.counterHitMoves[res.players[j].characterId][
                  res.players[j].conversions[k].moves[0].moveId
                ]++;
              }
              break;
            case 'trade':
              if (res.players[j].conversions[k].moves[0]) {
                myMoveUsageArr.tradeMoves[res.players[j].characterId][
                  res.players[j].conversions[k].moves[0].moveId
                ]++;
              }
              break;
        ...
        // Action Counts
        myActionCountArr[res.players[j].characterId][0] +=
          res.players[j].actionCounts.wavedashCount;
        myActionCountArr[res.players[j].characterId][1] +=
          res.players[j].actionCounts.wavelandCount;
        myActionCountArr[res.players[j].characterId][2] +=
          res.players[j].actionCounts.airDodgeCount;
        myActionCountArr[res.players[j].characterId][3] +=
          res.players[j].actionCounts.dashDanceCount;
        myActionCountArr[res.players[j].characterId][4] +=
          res.players[j].actionCounts.spotDodgeCount;
        myActionCountArr[res.players[j].characterId][5] +=
          res.players[j].actionCounts.ledgegrabCount;
        myActionCountArr[res.players[j].characterId][6] +=
          res.players[j].actionCounts.rollCount;
      } else {
        ...
        oppTally[current][2][res.players[j].characterId] =
          res.players[j].characterColor;
        if (res.metadata.winner === connect_code) {
          oppTally[current][3]++;
        ...
        myOppCreditedKills += Number.isInteger(res.players[j].creditedKillCount)
          ? res.players[j].creditedKillCount
          : 0;
        ...
        grabCountSuccessOppCharUsage[res.players[j].characterId] +=
          res.players[j].throwCount.up +
          res.players[j].throwCount.forward +
          res.players[j].throwCount.back +
          res.players[j].throwCount.down;
        grabCountWhiffOppCharUsage[res.players[j].characterId] +=
          res.players[j].grabCount.fail;

        throwCountOppCharUsage[0][res.players[j].characterId] +=
          res.players[j].throwCount.up;
        throwCountOppCharUsage[1][res.players[j].characterId] +=
          res.players[j].throwCount.forward;
        throwCountOppCharUsage[2][res.players[j].characterId] +=
          res.players[j].throwCount.back;
        throwCountOppCharUsage[3][res.players[j].characterId] +=
          res.players[j].throwCount.down;

        groundTechCountOppCharUsage[0][res.players[j].characterId] +=
          res.players[j].groundTechCount.backward;
        groundTechCountOppCharUsage[1][res.players[j].characterId] +=
          res.players[j].groundTechCount.forward;
        groundTechCountOppCharUsage[2][res.players[j].characterId] +=
          res.players[j].groundTechCount.neutral;
        groundTechCountOppCharUsage[3][res.players[j].characterId] +=
          res.players[j].groundTechCount.fail;

        wallTechCountSuccessOppCharUsage[res.players[j].characterId] +=
          res.players[j].wallTechCount.success;
        wallTechCountFailOppCharUsage[res.players[j].characterId] +=
          res.players[j].wallTechCount.fail;

        if (j === 0) {
          sdOppCharUsage[res.players[j].characterId] +=
            res.players[j].deathCount - res.players[1].creditedKillCount;
          deathOppUsage[res.players[j].characterId] += Number.isInteger(
            res.players[1].creditedKillCount
          )
            ? res.players[1].creditedKillCount
            : 0;
        ...
        for (let k = 0; k < res.players[j].conversions.length; k++) {
          ...
          if (res.players[j].conversions[k].didKill) {
            if (res.players[j].conversions[k].moves[0]) {
              myOppMoveUsageArr.killMoves[res.players[j].characterId][
                res.players[j].conversions[k].moves[
                  res.players[j].conversions[k].moves.length - 1
                ].moveId
              ]++;
          ...
          switch (res.players[j].conversions[k].openingType) {
            case 'neutral-win':
              if (res.players[j].conversions[k].moves[0]) {
                myOppMoveUsageArr.neutralWinMoves[res.players[j].characterId][
                  res.players[j].conversions[k].moves[0].moveId
                ]++;
              }
              break;
            case 'counter-attack':
              if (res.players[j].conversions[k].moves[0]) {
                myOppMoveUsageArr.counterHitMoves[res.players[j].characterId][
                  res.players[j].conversions[k].moves[0].moveId
                ]++;
              }
              break;
            case 'trade':
              if (res.players[j].conversions[k].moves[0]) {
                myOppMoveUsageArr.tradeMoves[res.players[j].characterId][
                  res.players[j].conversions[k].moves[0].moveId
                ]++;
              }
              break;
        ...
        // Action Counts
        myOppActionCountArr[res.players[j].characterId][0] +=
          res.players[j].actionCounts.wavedashCount;
        myOppActionCountArr[res.players[j].characterId][1] +=
          res.players[j].actionCounts.wavelandCount;
        myOppActionCountArr[res.players[j].characterId][2] +=
          res.players[j].actionCounts.airDodgeCount;
        myOppActionCountArr[res.players[j].characterId][3] +=
          res.players[j].actionCounts.dashDanceCount;
        myOppActionCountArr[res.players[j].characterId][4] +=
          res.players[j].actionCounts.spotDodgeCount;
        myOppActionCountArr[res.players[j].characterId][5] +=
          res.players[j].actionCounts.ledgegrabCount;
        myOppActionCountArr[res.players[j].characterId][6] +=
          res.players[j].actionCounts.rollCount;
      }
    }
  }
  ...
  if (myoppcode) {
    myoppcode.charAt(0) === '!'
      ? excludeArr.push(myoppcode.slice(1))
      : playerArr.push(myoppcode);
  }
  ...
  let characters = [];
  characters =
    req.query.character !== undefined ? req.query.character : chararr;
  ...
  let oppcharacters = [];
  oppcharacters =
    req.query.oppcharacter !== undefined ? req.query.oppcharacter : chararr;
  ...
  let complete = '';
  complete =
    req.query.complete !== undefined ? req.query.complete : completearr;
  ...
  co(function* () {
    ...
    do {
      const cursor = Match.find({
        'players.code': { $all: playerArr, $nin: excludeArr }, // [geff#353]
        //'players.code': {  }, // []
        players: {
          $all: [
            {
              $elemMatch: {
                code: mycode,
                characterString: { $in: characters },
              },
            },
            {
              $elemMatch: {
                code: { $ne: mycode },
                characterString: { $in: oppcharacters },
              },
            },
          ],
        },
    ...
    myOppOpeningsPerKO =
      myOppOpenings / myOppKills ? myOppOpenings / myOppKills : 0;

    // Conversion Rate
    myConversionRate = Math.round(
      (mySuccessfulConversions / myConversions) * 100
    )
      ? Math.round((mySuccessfulConversions / myConversions) * 100)
      : 0;
    myOppConversionRate = Math.round(
      (myOppSuccessfulConversions / myOppConversions) * 100
    )
      ? Math.round((myOppSuccessfulConversions / myOppConversions) * 100)
      : 0;

    // Damage Per Opening
    myAvgDamagePerOpening = Math.round(myDamage / myOpenings)
      ? Math.round(myDamage / myOpenings)
      : 0;
    myOppAvgDamagePerOpening = Math.round(myOppDamage / myOppOpenings)
      ? Math.round(myOppDamage / myOppOpenings)
      : 0;

    // Avg KO Percent
    myAvgKOpercent = Math.round(myDamage / myKills)
      ? Math.round(myDamage / myKills)
      : 0;
    myOppAvgKOpercent = Math.round(myOppDamage / myOppKills)
      ? Math.round(myOppDamage / myOppKills)
      : 0;
    ...
    myAvgStockDifferential =
      myTotalStockDifferential / myTotalWins
        ? myTotalStockDifferential / myTotalWins
        : 0;
    myOppAvgStockDifferential =
      myOppTotalStockDifferential / myTotalLosses
        ? myOppTotalStockDifferential / myTotalLosses
        : 0;
    ...
    for (let i = 0; i < quitoutmyCharUsage.length; i++) {
      quitoutmyCharUsagePercent[i] =
        quitoutmyCharUsage[i] / myCharUsage[i]
          ? Math.round((quitoutmyCharUsage[i] / myCharUsage[i]) * 100)
          : 0;
      quitoutmyOppCharUsagePercent[i] =
        quitoutmyOppCharUsage[i] / myOppCharUsage[i]
          ? Math.round((quitoutmyOppCharUsage[i] / myOppCharUsage[i]) * 100)
          : 0;
    }
    ...
    var resObj = {
      ...
      rivalsCharId: oppTally[items[0][0]][1].indexOf(
        Math.max(...oppTally[items[0][0]][1])
      ),
      rivalsColorId:
        oppTally[items[0][0]][2][
          oppTally[items[0][0]][1].indexOf(
            Math.max(...oppTally[items[0][0]][1])
          )
        ],
...
exports.getPlayers = (req, res) => {
  res.header('Access-Control-Allow-Origin', '*');
  Match.distinct('players.code').then((data) => {
