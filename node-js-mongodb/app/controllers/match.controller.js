...
exports.findAll = (req, res) => {
    ...
    if (!res.settings.isTeams) {
      ...
      for (let j = 0; j < res.players.length; j++) {
        if (res.players[j].code === connect_code) {
          ...
          myCharColor[res.players[j].characterId] =
          ...
          myOppCreditedKills += Number.isInteger(
            res.players[j].creditedKillCount
          )
            ? res.players[j].creditedKillCount
            : 0;
  ...
  co(function* () {
    ...
    myConversionRate = parseInt((mySuccessfulConversions / myConversions) * 100)
      ? parseInt((mySuccessfulConversions / myConversions) * 100)
      : 0;
    myOppConversionRate = parseInt(
      (myOppSuccessfulConversions / myOppConversions) * 100
    )
      ? parseInt((myOppSuccessfulConversions / myOppConversions) * 100)
      : 0;

    // Damage Per Opening
    myAvgDamagePerOpening = parseInt(myDamage / myOpenings)
      ? parseInt(myDamage / myOpenings)
      : 0;
    myOppAvgDamagePerOpening = parseInt(myOppDamage / myOppOpenings)
      ? parseInt(myOppDamage / myOppOpenings)
      : 0;

    // Avg KO Percent
    myAvgKOpercent = parseInt(myDamage / myKills)
      ? parseInt(myDamage / myKills)
      : 0;
    myOppAvgKOpercent = parseInt(myOppDamage / myOppKills)
      ? parseInt(myOppDamage / myOppKills)
      : 0;
    ...
    for (let i = 0; i < quitoutmyCharUsage.length; i++) {
      quitoutmyCharUsagePercent[i] =
        quitoutmyCharUsage[i] / myCharUsage[i]
          ? parseInt((quitoutmyCharUsage[i] / myCharUsage[i]) * 100)
          : 0;
      quitoutmyOppCharUsagePercent[i] =
        quitoutmyOppCharUsage[i] / myOppCharUsage[i]
          ? parseInt((quitoutmyOppCharUsage[i] / myOppCharUsage[i]) * 100)
          : 0;
    }
...
exports.getAllMatches = (req, res) => {
  ...
  let charStageWins = Array.from(
    { length: 26 }, // char
    (e) =>
      Array.from(
        { length: 26 }, // vs what char
        (e) => Array(33).fill(0)
      ) // what stage
  );

  let charStageLoss = Array.from(
    { length: 26 }, // char
    (e) =>
      Array.from(
        { length: 26 }, // vs what char
